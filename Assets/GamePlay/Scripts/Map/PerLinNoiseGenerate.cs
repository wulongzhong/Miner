...
public class PerLinNoiseGenerate {
    ...
    public PerLinNoiseGenerate(int seed) {
        ...
        RandomInfo[] arrayRandomInfo = new RandomInfo[m_oldPerm.Length];

        for(int i = 0; i < m_oldPerm.Length; ++i) {
        ...
        m_arrayPermutation = new int[512];
        for (int x = 0; x < 512; x++) {
            m_arrayPermutation[x] = m_oldPerm[arrayRandomInfo[x % 256].index];
        }
    }
